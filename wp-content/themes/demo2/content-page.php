...
<div class="article" id="post-<?php the_ID(); ?>" <?php post_class(); ?> role="article">
	...
	<?php if(has_post_thumbnail()): ?>
		...
		<?php if ($thumbnail_width > $min_width): ?>
			<div class="article-image">
				<?php the_post_thumbnail('citeo-full', array('class' => 'img-responsive', 'width' => '100%', 'height' => 'auto', 'alt' => get_the_title() )); ?>
			</div>
		<?php else: ?>
	...
	<?php if( get_field('citeo_extra_documents') || get_field('citeo_extra_links') || get_field('citeo_extra_ressources') || get_field('citeo_extra_contacts') ): ?>
		<?php get_template_part( 'parts/template', 'extras-infos' ); ?>
	<?php endif; ?>

	<?php if( get_field('citeo_extra_related_content') ): ?>
		<?php get_template_part( 'parts/template', 'related' ); ?>
	<?php endif; ?>
